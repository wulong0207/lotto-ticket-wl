...
public class HttpUtil {
	 ...
	 * @Desc 代理IP
	 */
	private static final String HTTP_PROXYHOST = PropertyUtil.getPropertyValue("sys_dynamic.properties",
			"http_proxyHost");

	/**
	 * 代理端口
	 */
	private static final String HTTP_PROXYPORT = PropertyUtil.getPropertyValue("sys_dynamic.properties",
			"http_proxyPort");
	/**
	 * 不使用代理的白名单
	 */
	private static final String HTTP_NONPROXYHOSTS = PropertyUtil.getPropertyValue("sys_dynamic.properties",
			"http_nonProxyHosts");
	/**
	 * 是否启用代理
	 */
	private static final String HTTP_PROXY_ENABLE = PropertyUtil.getPropertyValue("sys_dynamic.properties",
			"http_proxy_enable");
	...
	public static String doGet(CloseableHttpClient httpClient, String url, Map<String, String> params,
		...
		try {
			// 创建httpclient
			if (httpClient == null) {
				httpClient = createProxyClient(url);
	 ...
	 * @author jiangwei
	 * @Version 1.0
	 * @CreatDate 2017年8月14日 下午4:49:11
	 * @param url
	 * @param charset
	 * @param paramsObj
	 * @return
	 ...
	 * @throws URISyntaxException
	 */
	public static String doPostCharset(String url, String charset, Object paramsObj)
			throws IOException, URISyntaxException {
		return doPost(null, url, paramsObj, null, charset, true);
	}
	...
	private static CloseableHttpClient createProxyClient(String url) {
	 ...
	 * @author jiangwei
	 * @Version 1.0
	 * @CreatDate 2018年3月14日 上午9:24:18
	 * @param url
	 * @param data
	 * @param contentType
	 * @return
	...
	public static String doPostStream(String url, String data, String contentType) throws Exception {
		byte[] sendData = data.getBytes(DEFAULT_CHARSET);
		HttpClient hc = new HttpClient();
		PostMethod pm = new PostMethod(url);
		if (isProxy(url)) {
			// hc.getHostConfiguration().setProxy(HTTP_PROXYHOST,Integer.parseInt(HTTP_PROXYPORT));
			// hc.getParams().setAuthenticationPreemptive(true);

			AuthScope authScope = new AuthScope(HTTP_PROXYHOST, Integer.parseInt(HTTP_PROXYPORT));
			Credentials credentials = new UsernamePasswordCredentials("", "");
			hc.getState().setProxyCredentials(authScope, credentials);
		}
		hc.setConnectionTimeout(5000);
		hc.setTimeout(DEFAULT_SOCKETTIMEOUT);
		RequestEntity se = new ByteArrayRequestEntity(sendData, contentType);
	...
	@SuppressWarnings("deprecation")
	public static String doPostArray(String url, String paramName, String[] param, Map<String, String> paramOther,
		...
		client.setTimeout(DEFAULT_SOCKETTIMEOUT);
		HttpMethodParams p = new HttpMethodParams();
		p.setContentCharset(DEFAULT_CHARSET);
		if(paramOther != null){
			for (Map.Entry<String, String> entry : paramOther.entrySet()) {
				method.addParameter(entry.getKey(), entry.getValue());
			}
		}
		for (String string : param) {
			method.addParameter( new org.apache.commons.httpclient.NameValuePair(paramName,string));
		}
		//p.setParameters(param, paramName);
		method.setParams(p);
		try {
